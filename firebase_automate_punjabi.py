...
with open(PATH+'youtube_code_punjabi.txt') as f:
...
try:
    ...
    if len(d) > 0:
        with open(PATH+'youtube_code_punjabi.txt', 'w') as f:
            for item in t:
                f.write('%s\n' % item)

        with open(PATH+'youtube_title_punjabi.txt', 'w') as f:
            for item in d:
                f.write('%s\n' % item)

        firebase_conn_punjabi.call()
